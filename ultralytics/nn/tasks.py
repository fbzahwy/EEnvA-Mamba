...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, argsi：当前层的索引。f：当前层的输入来源。n：当前层的数量。m：当前层的模块名称。args：当前层的参数。
        ...
        if m in {
            ...
            C3x,
            RepC3,
            AStem, FConv, AVSSBlock, AXSSBlock
        }:
            c1, c2 = ch[f], args[0]
            ...
            args = [c1, c2, *args[1:]]
            if m in {BottleneckCSP, C1, C2, C2f, C2fAttn, C3, C3TR, C3Ghost, C3x, RepC3, AXSSBlock}:
