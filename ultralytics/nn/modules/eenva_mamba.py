from .common_utils_eenvamamba import *
from .conv import Conv, DWConv, GhostConv, LightConv, RepConv, autopad
import torch.nn.functional as F

__all__ = ("AVSSBlock", "AStem", "FConv", "AXSSBlock")
...
class GcBottleneck(nn.Module):
    """Standard bottleneck with GhostConv."""
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):
        ...
        self.cv1 = GhostConv(c1, c_, k[0], 1)  # Replace Conv with GhostConv
        self.cv2 = GhostConv(c_, c2, k[1], 1, g=g)  # Replace Conv with GhostConv
...
class GcC2f(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)  # Keep normal Conv for early feature extraction
        self.cv2 = Conv((2 + n) * self.c, c2, 1)  # Keep normal Conv for early feature extraction
        self.m = nn.ModuleList(GcBottleneck(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
...
class gMlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.,
        ...
        # Linear = Linear2d if channels_first else nn.Linear
        self.fc1 = nn.Conv2d(in_features, 2 * hidden_features, kernel_size=1)
        self.act = act_layer()
        self.fc2 = nn.Conv2d(hidden_features, out_features, kernel_size=1)
        self.drop = nn.Dropout(drop)
...
class EEnvA_MLP(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.,
        ...
        self.noise_aware = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(hidden_features, hidden_features, kernel_size=1, groups=hidden_features),
            nn.Sigmoid()
        )

        self.dynamic_gate = nn.Conv2d(hidden_features, hidden_features, kernel_size=3, padding=1,
                                      groups=hidden_features)

        self.fc4 = nn.Conv2d(hidden_features, out_features, kernel_size=1)

        self.drop = nn.Dropout(drop)
        self.act = act_layer()

    def forward(self, x):
        t, v = self.fc1(x).chunk(2, dim=1)
        noise_mask = self.noise_aware(v)
        v = v * noise_mask
        t = self.act(self.dynamic_gate(t)) + t
        t = t * v
        t = self.drop(t)
        x = self.fc4(t)
        x = self.drop(x)
...
class AXSSBlock(nn.Module):
    def __init__(
        ...
        #     nn.Conv2d(2*hidden_dim, hidden_dim, kernel_size=1, stride=1, padding=0, bias=False),
        #     nn.BatchNorm2d(hidden_dim),
        #     nn.SiLU()
        # ) if in_channels != hidden_dim else nn.Identity()

        self.hidden_dim = hidden_dim
        self.hid_2 = hidden_dim // 2
        # ==========SSM============================
        self.norm = norm_layer(hidden_dim)
        # 改了ssd中self.hidden_dim
        self.ss2d = nn.Sequential(*(SS2D(d_model=hidden_dim,
                                         d_state=ssm_d_state,
                                         ssm_ratio=ssm_ratio,
        ...
        if self.mlp_branch:
            ...
            self.mlp = EEnvA_MLP(in_features=hidden_dim, hidden_features=mlp_hidden_dim, act_layer=mlp_act_layer,
                                       drop=mlp_drop_rate)
...
class AVSSBlock(nn.Module):
    def __init__(
        ...
        #     nn.Conv2d(2*hidden_dim, hidden_dim, kernel_size=1, stride=1, padding=0, bias=True),
        #     nn.BatchNorm2d(hidden_dim),
        #     nn.SiLU()
        # )
...
class AStem(nn.Module):
    def __init__(self, inp, embed_dim, ks=3):
        ...
        self.channel_fc = nn.Conv2d(self.hidden_dims, self.hidden_dims, 1, 1, 0, bias=True)
        self.channel_act = nn.Sigmoid()

        spatial_ks = 7
        spatial_padding = 3 if spatial_ks == 7 else 1
        self.spatial_conv = nn.Conv2d(2, 1, spatial_ks, padding=spatial_padding, bias=False)
    ...
    def forward(self, x):
        x = self.conv1(x)
        channel_att = self.channel_act(self.channel_fc(self.channel_pool(x)))
        x = x * channel_att
        spatial_avg = torch.mean(x, 1, keepdim=True)
        spatial_max = torch.max(x, 1, keepdim=True)[0]
        spatial_att = self.spatial_act(self.spatial_conv(torch.cat([spatial_avg, spatial_max], 1)))
        x = x * spatial_att
        x = self.conv2(x)
        return x
